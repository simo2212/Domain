...
function initCanvas(canvas) {
	...
	canvasEl.addEventListener("mousemove", (event) => {
		...
		TweenMax.to(shadersValues.origin, 0.3, {
			x: obj.x - 0.5,
			ease: Elastic.easeOut,
			onUpdate: updateUniforms.bind(this)
		});
		TweenMax.to(shadersValues.origin, 0.8, {
			y: -(obj.y - 0.5),
			ease: Elastic.easeOut,
			overwrite: false,
			onUpdate: updateUniforms.bind(this)
	...
	canvas.setUniform(
		"u_tex0",
		"https://res.cloudinary.com/dnafoaa32/image/upload/v1499363239/dubai-37075265-1494255242-ImageGalleryLightboxLarge_dlsj3r.jpg"
	);
}

let canvasEl = document.querySelector("canvas");
let canvas = new GlslCanvas(canvasEl, {
